
Presentation of the scopeornot library

NOTICE

  This presentation has moved to:
  https://github.com/eric-brechemier/scopeornot/tree/master/doc

LANGUAGES

  OpenDocument Presentation

DESCRIPTION

  This project contains all the source files for the presentation of the
  scopeornot library [1] for the ParisJS [2] meetup on December 21st, 2011.

REFERENCES

  [1] scopeornot library
  https://github.com/eric-brechemier/scopeornot

  [2] ParisJS - JavaScript user group in Paris
  http://parisjs.org/

AUTHOR

  Eric Bréchemier <github@eric.brechemier.name>

LICENSE

  Slide template adapted from "tokyo-midtown-1.otp"
  http://technology.chtsai.org/impress/
  CC-BY-SA Chih-Hao Tsai

  Creative Commons Attribution-ShareAlike 3.0 Unported License
  http://creativecommons.org/licenses/by-sa/3.0/

COPYRIGHT

  Eric Bréchemier (c) 2011, Some Rights Reserved.

HISTORY

  2011-12-21, First version presented at ParisJS on December 21st, 2011
  2014-06-30, Merged with https://github.com/eric-brechemier/scopeornot/

